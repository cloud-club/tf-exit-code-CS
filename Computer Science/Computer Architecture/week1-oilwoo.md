...
실무 경험:
- 회사에서 jNIC를 적용한 Java 애플리케이션과 적용하지 않은 애플리케이션의 성능을 비교하는 벤치마크를 진행했습니다.
- 문서를 이미지로 변환하는 API를 약 1,000회 호출했을 때, 후반부 요청의 성능이 더 좋아지는 현상을 확인했습니다. JVM이 반복 호출되는 코드를 식별하고 JIT 컴파일을 통해 최적화하는 워밍업 과정이 성능 향상에 영향을 주었을 가능성이 있다고 판단했습니다.
- 따라서 두 애플리케이션을 공정하게 비교하려면 초기 워밍업 요청을 측정에서 제외하고 정상 상태(steady state)의 성능을 비교해야 합니다. 또한 캐시, GC, 커넥션 풀 등 다른 요인도 함께 통제해야 JIT의 영향을 더 정확히 판단할 수 있습니다.
